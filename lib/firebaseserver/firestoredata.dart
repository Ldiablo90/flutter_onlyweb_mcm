...
class FireStoreData {
  ...
  static Future<List<EntryModel>> getEntrysData(String type) async {
  ...
  static Future<List<DrinkModel>> getAllDrinks() async {
    ...
    final querySnapshot = await _drinkColection.get();
    if (querySnapshot.docs.isNotEmpty) {
      drinkInstances = querySnapshot.docs
          .map<DrinkModel>(
              (e) => DrinkModel.fromJson({"id": e.id, ...e.data()}))
          .toList();
    }
  ...
  static Future<List<MarketModel>> getAllMarkets() async {
    List<MarketModel> marketInstances = [];
    final querySnapshot = await _marketColection.get();
    if (querySnapshot.docs.isNotEmpty) {
      marketInstances = querySnapshot.docs
          .map<MarketModel>(
              (e) => MarketModel.fromJson({"id": e.id, ...e.data()}))
          .toList();
    }
  ...
  static Future<bool> entryWriteData(
      {required String drinkid,
      required String marketid,
      required DateTime releasedate,
      required String entrytype,
      required String price}) async {
    bool entryInstances = false;
    final today = DateTime.now();
    final int pricetemp = int.parse(price);
    final data = {
      'drinkid': drinkid,
      'marketid': marketid,
      'type': entrytype,
    ...
    print(data);
    try {
      await _entryColection.add(data);
      entryInstances = true;
    } catch (e) {
      print('파이어베이스 저장 애러');
    }
  ...
  static Future<bool> drinkWriteData(
    ...
    bool drinkInstances = false;
    final path = 'drink/${imagefile!.name}';
    final file = await imagefile.readAsBytes();
    final ref = _storage.ref(path);
    final snapshot = await ref.putData(file);
    final url = await snapshot.ref.getDownloadURL();
    final today = DateTime.now();
    final int alcoholtemp = int.parse(alcohol);
    final data = {
      "name": name,
      "name_en": nameen,
      "place": place,
    ...
    try {
      await _drinkColection.add(data);
      drinkInstances = true;
    } catch (e) {
      print('파이어베이스 저장 애러');
    }
  ...
  static Future<bool> marketWriteData({
    required String name,
    required String col,
    required String opening,
  }) async {
    bool marketInstances = false;
    final today = DateTime.now();
    final data = {
      "name": name,
      "place": '',
      "col": col,
      "image": 'https://placekitten.com/g/100/40',
      "opening": opening,
      "createdate": today,
    };
    // print(data);
    try {
      await _marketColection.add(data);
      marketInstances = true;
    } catch (e) {
      print('파이어베이스 저장 애러');
    }
    return marketInstances;
